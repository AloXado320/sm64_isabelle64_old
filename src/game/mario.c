...
void mario_reset_bodystate(struct MarioState *m) {
    ...
    bodyState->eyeState = ISABELLE_EYES_BLINK;
    bodyState->mouthState = ISABELLE_MOUTH_CLOSED;
...
void change_isabelle_face_expression(struct MarioState *m, s8 eye, s8 mouth) {
    struct MarioBodyState *bodyState = m->marioBodyState;

    bodyState->eyeState = eye;
    bodyState->mouthState = mouth;
}
...
void init_mario_from_save_file(void) {
    ...
    init_saved_alternate_costume(save_file_get_alt_costume());
}
