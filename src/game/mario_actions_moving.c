...
s32 act_hold_heavy_walking(struct MarioState *m) {
    ...
    change_isabelle_face_expression(m, ISABELLE_EYES_WORRIED, ISABELLE_MOUTH_VERY_OPEN);
...
s32 act_soft_forward_ground_kb(struct MarioState *m) {
    common_ground_knockback_action(m, MARIO_ANIM_SOFT_FRONT_KB, 32, FALSE, m->actionArg);
    change_isabelle_face_expression(m, ISABELLE_EYES_SAD, ISABELLE_MOUTH_SAD);
    return FALSE;
}

s32 act_ground_bonk(struct MarioState *m) {
    s32 animFrame = common_ground_knockback_action(m, MARIO_ANIM_GROUND_BONK, 0x20, TRUE, m->actionArg);
    change_isabelle_face_expression(m, ISABELLE_EYES_SAD, ISABELLE_MOUTH_SAD);
    if (animFrame == 32) {
...
// Won't fix BLJ in Isabelle 64, ever
//#ifdef VERSION_SH
