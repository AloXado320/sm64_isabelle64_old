...
void print_displaying_credits_entry(void) {
    ...
    if (sDispCreditsEntry != NULL) {
        ...
        strY = (sDispCreditsEntry->unk02 & 0x20 ? 208 : 48);

        lineHeight = 16;

// original x pos for N64: 10        
        dl_rgba16_begin_cutscene_msg_fade();
        print_generic_str_ascii(CREDIT_TEXT_X_LEFT, strY, titleStr);

        switch (numLines) {
            case 4:
                print_generic_str_ascii(CREDIT_TEXT_X_LEFT, strY - 16, *currStrPtr++);
                numLines = 2;
                lineHeight = 24;
                break;
            case 5:
...
#ifdef VERSION_EU
            case 6:
                print_generic_str_ascii(CREDIT_TEXT_X_LEFT, strY - 32, *currStrPtr++);
                numLines = 3;
                break;
            case 7:
                print_generic_str_ascii(CREDIT_TEXT_X_LEFT, strY - 16, *currStrPtr++);
        ...
        while (numLines-- > 0) {
            //print_generic_str_ascii((320 - 48) - get_credits_str_width(*currStrPtr), strY, *currStrPtr);
            print_generic_str_ascii(CREDIT_TEXT_X_RIGHT - get_string_width_ascii(*currStrPtr), strY, *currStrPtr);
...
void general_star_dance_handler(struct MarioState *m, s32 isInWater) {
    ...
    if (m->actionState == 0) {
        switch (++m->actionTimer) {
            case 1:
                spawn_object(m->marioObj, 0, bhvCelebrationStar)->header.gfx.sharedChild = m->interactObj->header.gfx.sharedChild;
...
s32 act_star_dance(struct MarioState *m) {
    ...
    stop_and_set_height_to_floor(m);
    change_isabelle_face_expression(m, ISABELLE_EYES_HAPPY, ISABELLE_MOUTH_BIG_SMILE);
    return FALSE;
}
...
s32 act_quicksand_death(struct MarioState *m) {
    ...
    change_isabelle_face_expression(m, ISABELLE_EYES_WORRIED, ISABELLE_MOUTH_OPEN);
    return FALSE;
}
    ...
    switch (m->actionState) {
        ...
        case 2:
            ...
            if (!(animFrame < 109) && animFrame < 154) {
                m->marioBodyState->eyeState = ISABELLE_EYES_HALF_CLOSED;
            }
...
s32 act_death_exit(struct MarioState *m) {
    if (15 < m->actionTimer++
        ...
        m->healCounter = 31;
        change_isabelle_face_expression(m, ISABELLE_EYES_WORRIED, ISABELLE_MOUTH_SAD);
    }
    // one unit of health
    m->health = 0x0100;
    return FALSE;
...
s32 act_shocked(struct MarioState *m) {
    ...
    change_isabelle_face_expression(m, ISABELLE_EYES_DIZZY, ISABELLE_MOUTH_OPEN);
...
void stuck_in_ground_handler(struct MarioState *m, s32 animation, s32 unstuckFrame, s32 target2,
    ...
    stop_and_set_height_to_floor(m);
    change_isabelle_face_expression(m, ISABELLE_EYES_SAD, ISABELLE_MOUTH_SAD);
...
static u8 sMarioBlinkOverride[20] = {
    ISABELLE_EYES_HALF_CLOSED, ISABELLE_EYES_HALF_CLOSED, ISABELLE_EYES_CLOSED, ISABELLE_EYES_CLOSED,
    ISABELLE_EYES_HALF_CLOSED, ISABELLE_EYES_HALF_CLOSED, ISABELLE_EYES_OPEN,   ISABELLE_EYES_OPEN,
    ISABELLE_EYES_HALF_CLOSED, ISABELLE_EYES_HALF_CLOSED, ISABELLE_EYES_CLOSED, ISABELLE_EYES_CLOSED,
    ISABELLE_EYES_HALF_CLOSED, ISABELLE_EYES_HALF_CLOSED, ISABELLE_EYES_OPEN,   ISABELLE_EYES_OPEN,
    ISABELLE_EYES_HALF_CLOSED, ISABELLE_EYES_HALF_CLOSED, ISABELLE_EYES_CLOSED, ISABELLE_EYES_CLOSED,
};
...
static void end_peach_cutscene_kiss_from_peach(struct MarioState *m) {
    ...
    if (m->actionTimer >= 90) {
        m->marioBodyState->eyeState =
            m->actionTimer < 110 ? sMarioBlinkOverride[m->actionTimer - 90] : ISABELLE_EYES_HALF_CLOSED;
    }
    ...
    switch (m->actionTimer) {
        ...
        case 76:
            m->marioBodyState->eyeState = ISABELLE_EYES_CLOSED;
            break;
...
static void end_peach_cutscene_star_dance(struct MarioState *m) {
    ...
    switch (m->actionTimer) {
        ...
        if (gPlayer3Controller->buttonDown & A_BUTTON) {
            // insert function to play Isabelle's singing here
        } else {
