...
static void print_act_selector_strings(void) {
    ...
    switch (language) {
        ...
        case LANGUAGE_ITALIAN:
            ...
            levelNameTbl = segmented_to_virtual(course_name_table_eu_it);
            break;
        case LANGUAGE_SPANISH:
            actNameTbl = segmented_to_virtual(act_name_table_eu_es);
            levelNameTbl = segmented_to_virtual(course_name_table_eu_es);
            break;
    }
    currLevelName = segmented_to_virtual(levelNameTbl[gCurrCourseNum - 1]);
#endif
    ...
    if (sVisibleStars != 0) {
        ...
        print_generic_string(actNameX, 140, selectedActName);
    }
